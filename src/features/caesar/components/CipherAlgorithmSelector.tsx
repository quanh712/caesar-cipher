...
export function CipherAlgorithmSelector({ value, onChange }: CipherAlgorithmSelectorProps) {
  return (
    <section className="algorithm-section" aria-labelledby="algorithm-title">
      <div className="section-label" id="algorithm-title">Thuật toán</div>
      <div className="algorithm-selector" role="tablist" aria-label="Chọn thuật toán mật mã">
        {algorithms.map((algorithm) => (
          <button
            className={value === algorithm.value ? "algorithm-card algorithm-card--active" : "algorithm-card"}
            key={algorithm.value}
